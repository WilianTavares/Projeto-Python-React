...
const addTask = async(
    { code, title, description, due_date, employee_id, status_id }: { code: string, title: string, description?: string, due_date?: string, employee_id: number, status_id: number }
) => {
    const response = await useApi<ApiGetTask>('companies/tasks', 'POST', { code, title, description, due_date, employee_id, status_id });
    return response;
}
...
const editTask = async (
    ...
    const response = await useApi<ApiGetTask>(`companies/tasks/${id}`, 'PUT', { code, title, description, due_date, employee_id, status_id });
    return response;
}
